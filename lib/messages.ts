...
const WINNER_MESSAGES: MessageTemplate[] = [
  ...
  { id: 'w14', text: "🎪 *{winner} {ws} - {loser} {ls}* ({round})\n{lp} really thought {loser} was gonna do something this year. Clown behavior. {wp} takes the point." },
  ...
  { id: 'w18', text: "🏋️ *{winner} {ws} - {loser} {ls}* ({round})\n{wp} carrying their bracket like it's nothing. {lp} couldn't carry a conversation about basketball." },
  ...
  { id: 'w23', text: "🤡 *{winner} {ws} - {loser} {ls}* ({round})\n{lp} drafting {loser} was certainly... a choice. {wp} thanks you for the free point." },
  ...
  { id: 'w26', text: "🧠 *{winner} {ws} - {loser} {ls}* ({round})\n{wp} is playing chess while {lp} is playing checkers. And losing." },
  ...
  { id: 'w30', text: "🎬 *{winner} {ws} - {loser} {ls}* ({round})\nAnother episode of '{wp} Wins and {lp} Cries'. Ratings are through the roof." },
]
...
const UPSET_MESSAGES: MessageTemplate[] = [
  ...
  { id: 'u6', text: "💣 BOOM\n*{winner} ({seed_w}) {ws} - {loser} ({seed_l}) {ls}* ({round})\n{seed_w} over {seed_l}!!! {lp} thought they were safe picking a {seed_l}-seed. WRONG. {wp} is cackling." },
  ...
  { id: 'u10', text: "🗣️ EVERYONE GET IN HERE\n*{winner} ({seed_w}) {ws} - {loser} ({seed_l}) {ls}* ({round})\n{seed_w} over {seed_l}!!! {lp} punching drywall. {wp} punching the air in celebration." },
  { id: 'u11', text: "🪦 REST IN PEACE\n*{winner} ({seed_w}) {ws} - {loser} ({seed_l}) {ls}* ({round})\n{lp} trusted a {seed_l}-seed and got betrayed. {wp} is living proof that chaos is a ladder." },
  { id: 'u12', text: "🤮 ABSOLUTELY DISGUSTING UPSET\n*{winner} ({seed_w}) {ws} - {loser} ({seed_l}) {ls}* ({round})\n{lp} is physically ill. A {seed_w} over a {seed_l}?! {wp} might frame this scoreline." },
...
const BLOWOUT_MESSAGES: MessageTemplate[] = [
  ...
  { id: 'b5', text: "💀 *{winner} {ws} - {loser} {ls}* ({round})\nThey mercy-ruled a tournament game. {lp} in witness protection. {wp} living lavish." },
  ...
  { id: 'b8', text: "😵 *{winner} {ws} - {loser} {ls}* ({round})\nThis game was over at halftime. {lp}, your team gave up. Like you should give up on your bracket. Meanwhile {wp} feasts." },
  ...
  { id: 'b15', text: "💨 *{winner} {ws} - {loser} {ls}* ({round})\nSmoke. That's all that's left of {loser}. And {lp}'s dignity. {wp} inhales another W." },
]
...
const CLOSE_GAME_MESSAGES: MessageTemplate[] = [
  ...
  { id: 'c6', text: "🫠 *{winner} {ws} - {loser} {ls}* ({round})\n{lp} is literally melting. That was RIGHT THERE. {wp} scraped by on pure luck and vibes." },
  ...
  { id: 'c9', text: "😬 *{winner} {ws} - {loser} {ls}* ({round})\nThat ending had everyone clenching. {wp} can finally breathe. {lp} might never breathe again." },
...
const BOTH_PICKED_MESSAGES: MessageTemplate[] = [
  ...
  { id: 'bp8', text: "🔪 *{winner} {ws} - {loser} {ls}* ({round})\n{wp} just ended {lp}'s whole career with that matchup. The group chat is about to be TOXIC." },
...
const LOSER_ONLY_MESSAGES: MessageTemplate[] = [
  ...
  { id: 'lo3', text: "🕳️ *{winner} {ws} - {loser} {ls}* ({round})\n{loser} is done. {lp} is down a team with nothing to show for it. Pure L energy." },
  ...
  { id: 'lo8', text: "🪦 *{winner} {ws} - {loser} {ls}* ({round})\n{lp} watches another team fall. And for what? Nobody scored. Life is pain." },
]
...
const NO_PICK_MESSAGES: MessageTemplate[] = [
  { id: 'np1', text: "💀 *{winner} {ws} - {loser} {ls}* ({round})\n{loser} eliminated. Nobody picked them so nobody cares lmao." },
  ...
  { id: 'np5', text: "😐 *{winner} {ws} - {loser} {ls}* ({round})\nA game happened. Nobody gained. Nobody lost. Nobody cared." },
...
const STANDINGS_TAUNTS = [
  ...
  "{leader} winning so hard it's almost unfair. Almost. Keep suffering {last}.",
  "{leader} making this look easy. {last} making this look painful.",
  "{leader} drinking champagne. {last} drinking alone.",
]
...
export function generateGameMessage(
  ...
  return { text: text + '\n\n<https://bracket-steel.vercel.app/|🖥️ View Dashboard>', messageId: template.id }
}
